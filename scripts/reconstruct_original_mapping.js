...
/**
 * RECONSTRUCT ORIGINAL BLOG IMAGE MAPPING
 * Based on color block image names and blog topics
 */
async function reconstructOriginalMapping() {
    console.log('=== RECONSTRUCTING ORIGINAL BLOG IMAGE MAPPING ===\n');

    // This mapping matches each blog slug to its logical color block image
    // based on the image filenames and blog topics
    const originalMapping = {
        ...
        'portable-power-stations-emergency-backup-us': '/images/blog/generated/portable-power-stations-webp.webp',
    ...
    // Remove duplicate key (ultimate-us-emergency-home-resilience-2026 appears twice)
    delete originalMapping['ultimate-us-emergency-home-resilience-2026'];

    let successCount = 0;
    let errorCount = 0;

    console.log(`Total blogs to update: ${Object.keys(originalMapping).length}\n`);

    for (const [slug, image] of Object.entries(originalMapping)) {
...
reconstructOriginalMapping().catch(console.error);
