...
# Fetch 10 most recent businesses
recent_url = f"{supabase_url}/rest/v1/businesses?select=id,name,city,trade,verified,is_premium,tier,created_at,selected_locations&order=created_at.desc&limit=10"

try:
    response = requests.get(recent_url, headers=headers)
    if response.status_code == 200:
        data = response.json()
        print(f"10 Most Recent Businesses:")
