...
// Target Texas Cities
const TARGET_CITIES = [
    { name: 'Houston', state: 'TX', center: { lat: 29.7604, lng: -95.3698 }, radius: 30, gridDensity: 6 },
    { name: 'Austin', state: 'TX', center: { lat: 30.2672, lng: -97.7431 }, radius: 20, gridDensity: 6 },
    { name: 'San Antonio', state: 'TX', center: { lat: 29.4241, lng: -98.4936 }, radius: 25, gridDensity: 6 }
];
...
async function collectData() {
    console.log("🤠 STARTING TEXAS DATA COLLECTION\n");
    console.log(`Targets: ${TARGET_CITIES.map(c => c.name).join(', ')}`);
    console.log("Output: JSON files for import\n");
    ...
    for (const city of TARGET_CITIES) {
        ...
        // To save time/cost for this run, let's limit grid for now or be comprehensive?
        // User wants "all details like before".
        // I will run fully.
