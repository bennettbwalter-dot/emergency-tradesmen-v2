...
print("--- Audit Bangor Data ---")
queries = ["Bangor", "Bangor (Wales)"]

for q in queries:
    res = supabase.table('businesses').select('*', count='exact').eq('city', q).execute()
    print(f"City: '{q}' | Count: {res.count}")
    if res.data:
        for b in res.data:
            print(f"  - {b.get('name')} ({b.get('trade')})")
