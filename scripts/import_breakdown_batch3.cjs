...
async function importTradeData(cities, tradeSlug) {
    for (const cityInfo of cities) {
        const { name: city, lat, lng, fileName: customFileName } = cityInfo;
        const normalizedCityName = city.toLowerCase().replace(/[^a-z0-9]/g, '');
        const fileName = customFileName || `${normalizedCityName}_${tradeSlug}_real.json`;
        ...
        const listings = rawData.map(item => ({
            ...
            trade: tradeSlug,
            city: city, // Use the provided name (e.g. "St. Paul")
            country_code: 'US',
...
const breakdownCities = [
    ...
    { name: "Sugar Land", lat: 29.5984, lng: -95.6226 },
    { name: "Allen", lat: 33.1032, lng: -96.6706 },
    { name: "League City", lat: 29.5075, lng: -95.0949 },
    { name: "Conroe", lat: 30.3119, lng: -95.4560 },
    { name: "New Braunfels", lat: 29.7030, lng: -98.1245 },
    { name: "Edinburg", lat: 26.3017, lng: -98.1633 },
    { name: "St. Paul", lat: 44.9537, lng: -93.0900, fileName: "stpaul_breakdown_real.json" },
    { name: "St. Louis", lat: 38.6270, lng: -90.1994, fileName: "stlouis_breakdown_real.json" }
];
