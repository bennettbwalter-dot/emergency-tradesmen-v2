...
/**
 * Aggressively removes dashes from provided text.
 */
...
// 2. Create the US version from cleaned UK text
const cleanUS = cleanUK
...
// 3. Inject the PROTECTED promo (which contains dashes in URLs but we want those kept!)
// Note: We already made bidetPromo have "is not" instead of "isn't" to avoid accidental cleaning if we ever re-run.
const finalUK = cleanUK.replace('[BIDET_PROMO_HERE]', bidetPromo);
const finalUS = cleanUS.replace('[BIDET_PROMO_HERE]', bidetPromo);
