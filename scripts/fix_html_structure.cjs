...
const getReq = https.request({
    ...
    res.on('end', () => {
        ...
        // Fix the malformed HTML at the end - remove extra closing divs
        const oldEnding = `</div>
</div>
</div>
</div>
        ...
        if (content.endsWith('</div>\n</div>\n</div>\n</div>\n</div>')) {
            content = content.substring(0, content.length - oldEnding.length) + newEnding;
            console.log('Fixed extra closing divs');
        } else {
            console.log('Content ending check - looking for extra divs');
