...
function generateSlug(name, city, trade) {
    /*
      Standardizing slug generation:
      - Lowercase
      - Replace non-alphanumeric chars with hyphens
      - Replace whitespace with hyphens
      - Trade suffix appended
    */
    return `${name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${city.toLowerCase().replace(/\s+/g, '-')}-${trade}`.replace(/--+/g, '-').replace(/^-|-$/g, '');
}

const cities = [
    { name: 'Towson', state: 'MD', filePrefix: 'towson_md' },
    { name: 'Columbia', state: 'MD', filePrefix: 'columbia_md' },
    { name: 'Glen Burnie', state: 'MD', filePrefix: 'glenburnie_md' }
];

const trades = ['plumber', 'electrician'];

async function importData() {
    console.log('Starting Baltimore Suburbs (Towson, Columbia, Glen Burnie) import...');
    let totalImported = 0;
    ...
    console.log(`\nTotal imported for Baltimore Suburbs: ${totalImported} listings`);
}
