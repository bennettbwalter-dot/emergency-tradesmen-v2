...
function generateSlug(name, city, trade) {
    return `${name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${city.toLowerCase()}-${trade}`.replace(/--+/g, '-').replace(/^-|-$/g, '');
}

const cities = [
    { name: 'Newark', state: 'NJ', filePrefix: 'newark_nj' }
];

const trades = ['plumber', 'electrician', 'locksmith', 'hvac', 'roofer', 'glazier'];

async function importData() {
    console.log('Starting Newark, NJ import...');
    let totalImported = 0;
    ...
    console.log(`\nTotal imported for Newark, NJ: ${totalImported} listings`);
}
