...
const getReq = https.request({
    ...
    res.on('end', () => {
        ...
        // Find and replace the img in amazon section with a simple test
        const regex = /<div class="product-image-wrapper">\s*<img[^>]+class="product-image"[^>]+\/?>\s*<\/div>/gi;
        const replacement = '<div class="product-image-wrapper"><img src="/images/blog/generated/portable-power-stations.png" alt="TEST" class="product-image"></div>';
        ...
        if (newContent !== content) {
            ...
            const payload = JSON.stringify({ content: newContent, updated_at: new Date().toISOString() });
