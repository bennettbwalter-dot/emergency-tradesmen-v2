...
const getReq = https.request({
    ...
    res.on('end', () => {
        ...
        // Find the exact img tag in regulatory section
        const regulatoryIndex = content.indexOf('regulatory-citation');
        if (regulatoryIndex > -1) {
            const section = content.substring(regulatoryIndex);
            const imgMatch = section.match(/<img[^>]+>/g);
            console.log('Images in regulatory section:');
            if (imgMatch) {
                imgMatch.forEach((img, i) => console.log(i + 1, img));
            }
        }
    });
});
