...
async function importTrades() {
    const filePath = path.join(__dirname, 'missing_water_data.json');
    ...
    console.log(`Found ${trades.length} missing trades to restore.`);
    ...
    for (const trade of trades) {
        ...
        if (error) {
            ...
            console.log(`Successfully restored: ${trade.name} (${trade.city})`);
            successCount++;
        }
    }
