...
function generateSlug(name, city, trade) {
    // Handle space removal for city slug generation if needed, but standardizing on hyphenated city name
    return `${name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${city.toLowerCase().replace(/\s+/g, '-')}-${trade}`.replace(/--+/g, '-').replace(/^-|-$/g, '');
}

const cities = [
    { name: 'King of Prussia', state: 'PA', filePrefix: 'kingofprussia_pa' },
    { name: 'Cherry Hill', state: 'NJ', filePrefix: 'cherryhill_nj' },
    { name: 'Main Line', state: 'PA', filePrefix: 'mainline_pa' }
];

const trades = ['plumber', 'electrician'];

async function importData() {
    console.log('Starting Philadelphia Suburbs (King of Prussia, Cherry Hill, Main Line) import...');
    let totalImported = 0;
    ...
    console.log(`\nTotal imported for Philadelphia Suburbs: ${totalImported} listings`);
}
