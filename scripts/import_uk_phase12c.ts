...
const DATA_FILE = path.join(__dirname, 'data', 'uk_phase12_group_c_data.json');
...
async function importData() {
    try {
        ...
        console.log(`Found ${businesses.length} businesses to import for Phase 12 Group C.`);

        // Hardcoded System User ID (created via create_temp_user.ts)
        ...
        for (const biz of businesses) {
            ...
            if (error) {
                console.error(`Error inserting ${biz.business_name}:`, error.message);
                failCount++;
            } else {
                console.log(`Imported: ${biz.business_name} (${biz.trade}) in ${biz.city}`);
                successCount++;
            }
        }
