...
/**
 * Safely removes dashes from text without affecting URLs or image syntax.
 */
function safeCleanDashes(text) {
    if (!text) return text;
    
    // We want to replace dashes in text, but NOT inside [text](url) or ![alt](url)
    // A simple way to do this for this specific blog is to skip lines that look like markdown assets/links
    // OR we can just replace dashes in the non-markdown-special parts.
    
    const lines = text.split('\n');
    const cleanedLines = lines.map(line => {
        // If it's an image or a link with a URL, don't touch the URL part
        if (line.trim().startsWith('![') || line.includes('](')) {
            // Find the link part [text](url)
            return line.replace(/(\[[^\]]*\]\()([^\)]*)(\))/g, (match, prefix, url, suffix) => {
                // Clean the prefix (the readable text part)
                const cleanPrefix = prefix.replace(/[—–]/g, ', ').replace(/--/g, ', ').replace(/-/g, ' ');
                return cleanPrefix + url + suffix;
            }).replace(/^(!\[)([^\]]*)(\]\()([^\)]*)(\))/g, (match, sign, alt, prefix, url, suffix) => {
                // Clean the alt text
                const cleanAlt = alt.replace(/[—–]/g, ', ').replace(/--/g, ', ').replace(/-/g, ' ');
                return sign + cleanAlt + prefix + url + suffix;
            });
        }
...
Many homeowners ignore a running toilet because "it is just a bit of water." However, in 2026, with rising utility costs and a global focus on sustainability, ignoring water waste is no longer an option.
...
Over time, minerals and "tank slime" build up on the rubber flapper or the rim it sits on. This prevents a watertight seal. The Fix: Turn off the water at the isolation valve, flush to empty the tank, and wipe the bottom of the flapper and the rim with a cloth. If the flapper feels stiff or cracked, it is time for a £10 replacement from the hardware store.
...
If your 5 minute fix did not do the trick, do not let your water bill spiral out of control. We connect you with local, vetted plumbers who can be at your door within the hour.

**Internal Links:**
* [Find Emergency Plumbers Near Me](https://emergencytradesmen.net/services/plumbing)
* [How to Find Your Stopcock Before It is Too Late](https://emergencytradesmen.net/blog/find-stopcock-emergency)
...
const usContentBase = ukContentBase
...
async function updatePost(slug, content) {
    ...
    const finalContent = safeCleanDashes(content);
...
async function run() {
    await updatePost('toilet-wont-stop-running-2026-fix-gb', ukContentBase);
    await updatePost('toilet-wont-stop-running-2026-fix-us', usContentBase);
}
