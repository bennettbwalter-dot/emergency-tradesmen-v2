// Script to generate plumber additions for all remaining cities
// This will be used to batch-add all cities efficiently
...
cities.forEach(city => {
    console.log(`
        plumber: Array.from({ length: 25 }, (_, i) => ({
            ...
            name: [
                "${city.name} Plumbing Services", "Emergency Plumbers ${city.name}", "${city.name} Heating & Plumbing",
                "24/7 Plumbing ${city.name}", "${city.name} Emergency Plumbers Ltd", "Quick Fix Plumbing ${city.name}",
                "${city.name} Plumbing & Heating Ltd", "Rapid Response Plumbers ${city.name}", "${city.name} Plumbing Experts",
                "Local Plumbers ${city.name}", "${city.name} Boiler & Plumbing", "Professional Plumbers ${city.name}",
                "${city.name} Drain & Plumbing", "Reliable Plumbing ${city.name}", "${city.name} Plumbing Solutions",
                "Express Plumbers ${city.name}", "${city.name} Bathroom & Plumbing", "Fast Plumbing ${city.name}",
                "${city.name} Gas & Plumbing", "Premier Plumbers ${city.name}", "${city.name} Plumbing Repairs",
                "Trusted Plumbers ${city.name}", "${city.name} Emergency Heating", "Quality Plumbing ${city.name}",
                "${city.name} Plumbing Contractors"
            ][i],
            ...
            ...([1, 6, 11, 18].includes(i) && { website: \`http://www.${city.slug.replace(/&/g, '')}\${['plumbers', 'heating', 'pro', 'gas'][Math.floor(i/6)]}.co.uk\` })
        })),
  `);
});
