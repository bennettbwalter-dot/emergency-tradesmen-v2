/**
 * Update hero image for sewer-backup-prevention-gb
 */
...
async function updatePost(id, cover_image) {
...
async function main() {
    const post = await fetchPost('sewer-backup-prevention-gb');
    if (!post) {
        console.log('Post not found');
        return;
    }

    console.log('Old cover_image:', post.cover_image);
    const newCoverImage = '/images/blog/sewer-backup-gb/sewer-backup-hero.png';
    
    const success = await updatePost(post.id, newCoverImage);
    if (success) {
        console.log('✓ Updated to:', newCoverImage);
    } else {
