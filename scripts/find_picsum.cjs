...
const getReq = https.request({
    ...
    res.on('end', () => {
        ...
        // Find where the picsum img is
        const idx = content.indexOf('picsum');
        if (idx > -1) {
            console.log('picsum found at position:', idx);
            console.log('Context:', content.substring(Math.max(0, idx - 100), idx + 200));
        } else {
            console.log('picsum not found');
        }
        
        // Find all amazon sections
        const idx2 = content.indexOf('amazon-product-section');
        console.log('\namazon-product-section found at:', idx2);
        if (idx2 > -1) {
            console.log('Context:', content.substring(idx2 - 50, idx2 + 500));
        }
    });
});
