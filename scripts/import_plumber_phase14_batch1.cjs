...
async function importData() {
    console.log(`🚀 Starting Phase 14 Batch 1 Plumber Import...`);

    for (const city of cities) {
        let fileNameBase = city.toLowerCase().replace(/\s/g, '');
        if (city === 'New York City') fileNameBase = 'nyc';
        if (city === 'Los Angeles') fileNameBase = 'la';
    ...
    console.log('✨ Batch 1 Plumber Import Complete!');
}
