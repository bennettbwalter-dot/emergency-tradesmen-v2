...
def safe_print(msg):
    """Print with ASCII fallback for Windows logging compatibility."""
    try:
...
# --- Configuration ---
# Load .env with proper path resolution
...
# REGION CONFIG
REGION_NAME = "WEST"
TARGET_STATES = ["CA", "WA", "OR", "NV", "AZ", "UT", "CO", "NM", "HI", "AK"]
    ...
    while True:
        for loc in all_locations:
            ...
            for trade in TRADES:
                ...
                safe_print(f"[GAP] {city}, {state} [{trade}]: Has {current_count}/5")
                stats["checked"] += 1
                    ...
                    try:
                        ...
                        try:
                            ...
                            if res.data:
                                ...
                                safe_print(f"      [OK] ADDED: {biz['name']} ({trade})")
                                stats["filled"] += 1
