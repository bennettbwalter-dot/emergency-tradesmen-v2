...
cities = []
try:
    with open('src/lib/usCityStates.ts', 'r') as f:
        for line in f:
...
print("Fetching US data from Supabase...")
while True:
    url = f"{supabase_url}/rest/v1/businesses?country_code=eq.US&select=id,city,trade&order=id&limit={limit}&offset={offset}"
...
print(f"Total US Cities: {len(cities)}")
print(f"Target Trades: {len(trades)}")
print(f"Total Required Pages: {total_permutations}")
print(f"Total Verified US Businesses: {total_businesses}")
