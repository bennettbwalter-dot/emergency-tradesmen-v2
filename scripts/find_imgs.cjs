...
const getReq = https.request({
    ...
    res.on('end', () => {
        ...
        if (bodyMatch) {
            ...
            const imgCount = (bodyContent.match(/<img/g) || []).length;
            console.log('Img tags in body:', imgCount);
            
            // Find where the img tags are
            let idx = bodyContent.indexOf('<img');
            while (idx !== -1) {
                const end = Math.min(idx + 150, bodyContent.length);
                console.log('\nImg at position', idx, ':');
                console.log(bodyContent.substring(idx, end));
                idx = bodyContent.indexOf('<img', idx + 1);
            }
        }
    });
});
