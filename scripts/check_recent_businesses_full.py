...
# Fetch 2 most recent businesses with ALL fields
recent_url = f"{supabase_url}/rest/v1/businesses?select=*&order=created_at.desc&limit=2"

try:
    response = requests.get(recent_url, headers=headers)
    if response.status_code == 200:
        data = response.json()
        print(f"2 Most Recent Businesses (All Fields):")
