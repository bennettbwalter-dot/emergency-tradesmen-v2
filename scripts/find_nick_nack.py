...
search_term = "nick nack"

for p in projects:
    print(f"\n--- Searching {p['name']} for '{search_term}' ---")
    headers = {
        "apikey": p['key'],
    ...
    try:
        url = f"{p['url']}/rest/v1/businesses?select=id,name,city,trade,verified,is_premium,created_at&name=ilike.*{search_term}*"
