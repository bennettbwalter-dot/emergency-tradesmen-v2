                            ...
                            <div className="max-w-[1000px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12">
                                ...
                                <div className="lg:col-span-12">
                                    <div className="prose prose-lg dark:prose-invert max-w-none 
                                        ...
                                        prose-img:rounded-xl prose-img:shadow-lg prose-img:border prose-img:border-border/50 prose-img:w-full
                                    ">
                                        <ReactMarkdown
                                            ...
                                            components={{
                                                ...
                                                table: ({ node, ...props }) => (
                                                    <div className="overflow-x-auto my-12 border border-border/60 rounded-xl shadow-xl bg-background/50 backdrop-blur-sm">
                                                        <table {...props} className="w-full text-base border-collapse" />
                                                    </div>
                                                ),
                                                thead: ({ node, ...props }) => (
                                                    <thead {...props} className="bg-secondary/40 border-b border-border/60" />
                                                ),
                                                th: ({ node, ...props }) => (
                                                    <th {...props} className="px-6 py-4 text-left font-bold text-foreground uppercase tracking-widest text-xs" />
                                                ),
                                                td: ({ node, ...props }) => (
                                                    <td {...props} className="px-6 py-5 border-t border-border/40 text-muted-foreground/90 transition-colors hover:bg-gold/5" />
                                                ),
                                                tr: ({ node, ...props }) => (
                                                    <tr {...props} className="transition-colors hover:bg-secondary/20" />
                                                ),
                                                // Custom link renderer for Amazon buttons
                                                a: ({ node, ...props }) => {
                                                    ...
                                                    if (isAmazon) {
                                                        return (
                                                            <a 
                                                                {...props} 
                                                                className="inline-block bg-[#f0c14b] border border-[#a88734] rounded-lg text-[#111] px-6 py-3 font-bold no-underline my-4 shadow-sm hover:bg-[#e2b13c] transition-colors"
                                                                style={{ backgroundColor: '#f0c14b', border: '1px solid #a88734', color: '#111', textDecoration: 'none', display: 'inline-block' }}
                                                            >
                                                                {props.children}
                                                            </a>
                                                        );
                                                    }
                                                    return <a {...props} className="text-gold hover:underline" />;
                                                }
                                            }}
                                        >
                                            {post.content || ''}
